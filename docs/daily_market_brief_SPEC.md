...
| Movers | Top 3 to 5 gainers and losers from the finished session plus a labeled pre-market read, with a one-line reason, drawn from a curated liquid universe plus the watchlist, gated behind a volume floor. Best-effort: defaults to watchlist-movers-only and upgrades when universe data is reliable. | The most actionable single-stock section. | Horizontal bar of percent moves (optional) |
...
**The look.** Editorial gravitas without marketing gloss. One accent, not a palette.

- **Palette (hex):** ink navy `#13202E` for structure and the masthead; paper `#FBFAF7` for the page; card white `#FFFFFF` for the table; hairline `#E4E0D7`; a single gold rule `#B0892F`; direction green `#197A4B`; direction red `#BC3B2E`; muted grey `#6B7785` for captions and flat readings.
- **Type, with the signature:** a serif for the masthead (editorial feel), a clean sans for body, and a **monospace with tabular numerals for every figure** so numbers align in a column like a terminal. The tabular monospace numerals are the signature element; protect them. Design-target faces are Spectral (masthead), Inter (body), IBM Plex Mono (numbers).
- **Email-safe build constraints.** The design target above is what it should look like; the shipped template is constrained. Email clients strip `<style>` blocks, ignore flex and grid, and block web fonts. Therefore the production `template.html.j2` uses a single-column table layout, fully inline styles, and a web-safe font stack: Georgia (or a serif fallback) for the masthead and a `Consolas, "SFMono-Regular", monospace` stack for figures. The look survives this stack; the web fonts are a nice-to-have, not a dependency.
...
- **Color discipline.** Green and red carry direction only. Everything else is navy, paper, grey, and the one gold rule. Resist adding a second accent.
...
- **Upgrade:** when the curated-universe screen is reliable on a given morning, it upgrades the section to the fuller gainers-and-losers list. Universe is a curated list of liquid names (roughly 50 to 150) plus the watchlist, defined in config. Do not screen the full index live; it is slow and rate-limit prone.
