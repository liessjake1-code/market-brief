"""Tolerant number validator (spec §5.6; execution guide Part 4.4; roadmap §6.7).

LOAD-BEARING and built FIRST, before any model integration. This is what makes an
invented number fail to ship. The model is instructed to round and approximate
("about 76 dollars", never "76.23"), so the validator's job is "is this rounded
figure CONSISTENT WITH an input number" — not digit-for-digit identity.

An exact-match check would reject normal rounded prose every day and collapse the
brief into templates, so the tolerance is mandatory (spec §5.6).

Algorithm (Part 4.4):
  1. Extract every numeric token from the prose.
  2. Skip a whitelist entirely: clock times, calendar dates, spelled ordinals.
  3. For each remaining number, round both it and every candidate input to the
     same precision and accept within a tolerance band (±tol% for percentages,
     a small absolute band for prices, ±1 for bps).
  4. Match against the FULL input set, including derived figures (weekly sums,
     spreads, index gaps). A number matching nothing is rejected.
  5. Rejection is the caller's signal to retry once, then template.
"""
